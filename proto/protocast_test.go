...
func BenchmarkCopy(b *testing.B) {
	...
	if !reflect.DeepEqual(result, v) {
		b.Fatalf("bad: %#v", result)
	}
}

func BenchmarkCopy_byhand(b *testing.B) {
	type test struct {
		Value      string
		A, B, C, D uint64
	}
...
func TestPublicKey(t *testing.T) {
	privateKey, _ := ec.NewPrivateKey(ec.S256())
	publicKey := privateKey.PubKey()

	result, err := Copy(publicKey)
	if err != nil {
		t.Fatal(err)
	}

	if reflect.DeepEqual(publicKey, result) {
		t.Fatalf("\nn:%#v \nr:%#v \n", publicKey, result)
	}
}
